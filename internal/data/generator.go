...
func EngineeringScenario() []ScenarioEvent {
	return []ScenarioEvent{
		...
		{Content: "Service cron-beta log: Feature flag FF_ARCHIVE_SYNC is true. Including Archive data in sync.", Index: 45901},
		{Content: "Service cron-beta log: Payload size calculated at 5.2MB for current batch.", Index: 46500},
		{Content: "Service cron-beta log: Daily synchronization job started at 02:00:00 UTC to Service Omega /v1/legacy.", Index: 46901},
		{Content: "Envoy Release Notes: Known issue #442 - memory leak when non-standard X-Trace headers are present and payload exceeds 2MB.", Index: 89112},
		{Content: "Kubernetes Events: cgroup memory controller invoked OOM killer for pod service-omega-77x9.", Index: 110000},
		{Content: "Service Omega metrics: istio-proxy sidecar container terminated with reason OOMKilled at 02:48 UTC on endpoint /v1/legacy.", Index: 120555},
		{Content: "Service Omega metrics: OOM-kill triggered on 4 replicas at 02:48 UTC.", Index: 120556},
		{Content: "Service cron-beta status: Batch job failed due to connection reset from Service Omega at 02:48:12 UTC.", Index: 140000},
	}
}

func StreamUltraMassiveContext(w io.Writer, totalChunks int) error {
	noises := []string{
	...
	for _, e := range MedicalScenario() {
		eventMap[e.Index] = e.Content
	}
	for _, e := range EngineeringScenario() {
		eventMap[e.Index] = e.Content
	}
