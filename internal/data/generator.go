...
func EngineeringScenario() []ScenarioEvent {
        return []ScenarioEvent{
                ...
                {Content: "Envoy Release Notes: Known issue #442 - memory leak when non-standard X-Trace headers are present.", Index: 49112},
                {Content: "Service Omega metrics: OOM-kill triggered on 4 replicas at 02:48 UTC.", Index: 60555},
...
func GenerateUltraMassiveContext(totalChunks int) string {
        ...
        // Estimate size: ~50 bytes per line
        sb.Grow(totalChunks * 50)
