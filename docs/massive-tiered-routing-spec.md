...
## Purpose
To generate a synthetic dataset for testing the efficiency of agentic RAG against tiered model routing approaches, specifically targeting a context size that exceeds the 1M+ token window of `gemini-2.5+-pro` (and `2.5+-flash` equivalents). The dataset will focus on deep technical/engineering and medical scenarios, requiring complex, multi-hop reasoning across thousands of disparate chunks of information hidden within massive volumes of noise.
